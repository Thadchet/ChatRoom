...
const updateStoriesSuccess: ActionCreator<StoriesActionTypes> = (
...
const updateMyStorySuccess: ActionCreator<StoriesActionTypes> = (
    myStory: StoryInterface[]
) => {
    return { type: UPDATE_MY_STORY, payload: myStory };
};

const watchStorySuccess: ActionCreator<StoriesActionTypes> = (
    stories: StoryInterface[]
) => {
...
export function updateStories() {
    console.log("update Story action");
    return (dispatch) => {
        dispatch(request());
        return storiesService.updateStories().then(
            (response) => {
...
export function updateMyStory() {
    return (dispatch) => {
        dispatch(request());
        return storiesService.updateMyStory().then(
            (response) => {
...
export function watchStory({ storyId }: { storyId: String }) {
    return (dispatch) => {
        dispatch(request());
        return storiesService.watchStory({ storyId }).then(
            (response) => {
